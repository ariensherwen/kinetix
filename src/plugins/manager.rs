...
impl PluginManager {
    ...
    pub async fn install(
        ...
        // metadata. The package filename is content-addressed so untrusted
        // manifest version text never becomes a filesystem path component.
    ...
    async fn persist_package(&self, plugin_id: &str, sha256: &str, bytes: &[u8]) -> Result<String> {
        ...
        match tokio::fs::read(&target).await {
            Ok(existing) => {
                if existing != bytes {
                    bail!(
                        "plugin package store collision at {}: existing bytes do not match SHA-256 {}",
                        target.display(),
                        sha256
                    );
                }
    ...
    /// Exchange a callback code for a host-persistable account credential.
...
fn map_auth_result<T>(
    r: Result<T, crate::plugins::runtime::auth_bindings::kinetix::plugin::types::PluginError>,
) -> Result<T, PluginFault> {
    r.map_err(|e| PluginFault::PluginError {
        code: e.code,
        message: e.message,
