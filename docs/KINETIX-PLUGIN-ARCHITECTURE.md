...
Only immutable compiled code is reused. Kinetix creates a fresh Store and guest
instance for every invocation, so approved permissions, resource limits,
outbound counters, circuit state, per-plugin concurrency, metrics labels, and
epoch deadlines are always derived from current host state.

Global cache hit/miss counters remain available alongside the per-plugin
capability metrics.
