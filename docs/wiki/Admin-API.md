...
| `POST /admin/api/plugins/auth/start` | Start a named plugin account-auth flow for an existing provider binding. Requires admin auth; returns the provider authorization URL. |
| `GET /admin/api/plugins/auth/callback` | One-time provider callback authenticated by the expiring random `state` token. Exchanges the code inside WASM, encrypts the returned credential JSON, creates the account, and redirects to the Plugins page. |
